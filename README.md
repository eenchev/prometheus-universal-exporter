...
The exporter can collect metrics from almost any source: HTTP services returning JSON, YAML, XML, CSV, HTML, plain text, or Prometheus exposition; Graphite servers; or local files such as .prom files generated by batch jobs. Collectors define how to query, parse, transform, and publish the data as Prometheus metrics. Because everything is configuration-driven, collection rules can be changed without writing code, rebuilding the exporter, or redeploying it.
