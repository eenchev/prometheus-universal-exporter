...
Verbose self-metrics are configured in the exporter configuration rather than in chart values: set `web.self_metrics.verbose: true` inside `config.data.config.yaml` to add a series per collector, request URL and method. They are off by default because a request URL is an unbounded label value; tracking is capped at 1000 combinations and `http_exporter_request_series_capped` reports when that cap is reached. If you scrape the exporter with the self-metrics monitor, consider the extra cardinality before enabling it across a large target set.
