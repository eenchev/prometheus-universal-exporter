...
func TestCommittedConfigSchemaIsCurrent(t *testing.T) {
	...
	generated, err := configSchemaJSON()
	if err != nil {
		t.Fatal(err)
	}
...
func TestConfigSchemaFlagPrintsTheSchema(t *testing.T) {
	...
	generated, _ := configSchemaJSON()
	if out.code != 0 || out.stdout != string(generated) || out.stderr != "" {
		t.Fatalf("exit=%d stderr=%q stdout starts %q", out.code, out.stderr, firstLines(out.stdout, 3))
	}
...
func TestConfigSchemaCoversEveryKey(t *testing.T) {
	...
	walk(reflect.TypeOf(Config{}), configSchema(), "")
}
...
func TestConfigSchemaListsTheBuiltRequestTypes(t *testing.T) {
	...
	if !reflect.DeepEqual(requestType["enum"], builtRequestTypes()) {
		t.Fatalf("enum=%v, built=%v", requestType["enum"], builtRequestTypes())
	}
}

// The examples point editors at the published schema.
func TestExamplesReferenceTheSchema(t *testing.T) {
	modeline := "# yaml-language-server: $schema=" + configSchemaID
