...
func fetch(ctx context.Context, target string, c *Collector, forwarded ...http.Header) (*HTTPResponse,error) {
	...
	method:=c.Request.Method;reqBody:=io.Reader(nil);if c.Request.Body!=""{reqBody=strings.NewReader(c.Request.Body)};req,err:=http.NewRequestWithContext(ctx,method,u.String(),reqBody);if err!=nil{return nil,err};for k,v:=range c.Request.Headers{req.Header.Set(k,v)};if c.Request.BasicAuth!=nil{req.SetBasicAuth(c.Request.BasicAuth.Username,c.Request.BasicAuth.Password)};bearerToken:=c.Request.BearerToken;if c.Request.BearerTokenFile!=""{token,readErr:=os.ReadFile(c.Request.BearerTokenFile);if readErr!=nil{return nil,fmt.Errorf("reading bearer token file: %w",readErr)};bearerToken=strings.TrimSpace(string(token));if bearerToken==""{return nil,fmt.Errorf("bearer token file %s is empty",c.Request.BearerTokenFile)}};if bearerToken!=""{req.Header.Set("Authorization","Bearer "+bearerToken)};if len(forwarded)>0{for k,v:=range forwarded[0]{req.Header[k]=append([]string(nil),v...)}}
