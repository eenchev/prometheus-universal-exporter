...
// A localfile collector reading a directory (localfile_directory.go) hands
...
//
// The type lives outside the localfile build tag because the probe and
// scheduled-scrape paths that consume it are shared by every request type.

// DirectoryRead is what reading a directory produced.
...
func (s *Server) collectDirectory(ctx context.Context, read *DirectoryRead, c *Collector, rec statsRecorder, logTarget string) *MetricSet {
	...
	synthetic := func(name string, value float64, labels map[string]string) Metric {
		return Metric{Name: name, Help: localFileMetricHelp[name], Type: GaugeMetricType, Value: value, Labels: labels}
	}
	for _, file := range read.Files {
...
func (s *Server) collectFile(ctx context.Context, file FileRead, c *Collector, rec statsRecorder) (*MetricSet, *fileFailure) {
	if file.Err != nil {
		if errors.Is(file.Err, errLimitExceeded) {
			rec.update(func(x *serverStats) { x.limitErrors++ })
		}
		return nil, &fileFailure{"file", file.Err}
	}
	d, err := decode(file.Response, c)
	if err != nil {
		rec.update(func(x *serverStats) { x.parseErrors++ })
		return nil, &fileFailure{"decode", err}
	}
	rec.update(func(x *serverStats) { x.decodeOK++ })
	set, err := transform(ctx, d, file.Response, c, s.pythonPath)
	if err != nil {
...
// family only once.
func checkFileFamilies(set *MetricSet, families map[string][]Metric, typeFrom map[string]string) *fileFailure {
