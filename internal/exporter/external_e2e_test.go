...
func TestExternalDemoConfigurationsStillMatchTheirSources(t *testing.T) {
	...
	for _, test := range externalCases {
		t.Run(test.name, func(t *testing.T) {
			cfg, err := config.Load(test.config)
			if err != nil {
			...
			server := NewServer(config.NewManager(cfg, test.config, slog.Default()), "python3", slog.Default())
