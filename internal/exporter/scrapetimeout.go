...
func explainBudget(ctx context.Context, budget time.Duration, err error) error {
	if budget <= 0 || !errors.Is(ctx.Err(), context.DeadlineExceeded) {
		return err
	}
	return fmt.Errorf("%w (the probe ran out of its %s budget: Prometheus's scrape timeout less --probe.timeout-offset)", err, budget)
}
