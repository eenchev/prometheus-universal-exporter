...
func itemsSet(t *testing.T, transformType, contentType, body string, rules ...MetricRule) (*MetricSet, error) {
	t.Helper()
	c := Collector{Name: "items", Request: RequestConfig{Type: RequestTypeHTTP}, Transform: TransformConfig{Type: transformType}, Metrics: rules, Limits: Limits{MaxMetrics: 100}}
	if err := (&Config{Collectors: []Collector{c}}).Validate(); err != nil {
		t.Fatal(err)
	}
	r := &HTTPResponse{Body: []byte(body), Headers: http.Header{"Content-Type": {contentType}}}
	d, err := decode(r, &c)
	if err != nil {
		t.Fatal(err)
	}
...
func exprLabel(name, expression string) LabelRule {
	return LabelRule{Name: name, Type: "expression", Expression: expression}
}
...
func TestItemsEvaluatesEachItemOnItsOwn(t *testing.T) {
	set, err := itemsSet(t, "jq", "application/json", itemsDocument, MetricRule{
		Name: "server_cpu", Type: GaugeMetricType, Items: ".servers[]", Expression: ".cpu",
		Labels: []LabelRule{
...
func TestItemsWithYQ(t *testing.T) {
	set, err := itemsSet(t, "yq", "application/yaml", "servers:\n  - name: web01\n    cpu: 72\n  - name: web02\n    cpu: 18\n", MetricRule{
		Name: "server_cpu", Type: GaugeMetricType, Items: ".servers[]", Expression: ".cpu", Labels: []LabelRule{exprLabel("server", ".name")},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestItemsMissingValue(t *testing.T) {
	captureLogs(t)
	rule := MetricRule{Name: "server_cpu", Type: GaugeMetricType, Items: ".servers[]", Expression: ".load", Labels: []LabelRule{exprLabel("server", ".name")}}
	body := `{"servers": [{"name": "web01", "load": 3}, {"name": "web02"}, {"name": "web03", "load": null}]}`

	rule.ErrorMode = ErrorModeLog
	...
	notRequired := false
	rule.ErrorMode, rule.Required = ErrorModeFail, &notRequired
	set, err = itemsSet(t, "jq", "application/json", body, rule)
...
func TestItemsExpressionWithSeveralValues(t *testing.T) {
	captureLogs(t)
	_, err := itemsSet(t, "jq", "application/json", `{"servers": [{"name": "web01", "cpu": [1, 2]}]}`, MetricRule{
		Name: "server_cpu", Type: GaugeMetricType, ErrorMode: ErrorModeFail, Items: ".servers[]", Expression: ".cpu[]",
	})
	if err == nil || !strings.Contains(err.Error(), `expression ".cpu[]" produced 2 values for one item; it must produce at most one`) {
		t.Fatalf("err=%v", err)
	}
	_, err = itemsSet(t, "jq", "application/json", `{"servers": [{"name": ["a", "b"], "cpu": 1}]}`, MetricRule{
		Name: "server_cpu", Type: GaugeMetricType, ErrorMode: ErrorModeFail, Items: ".servers[]", Expression: ".cpu",
		Labels: []LabelRule{exprLabel("server", ".name[]")},
	})
...
func TestItemsSelectingNothing(t *testing.T) {
	captureLogs(t)
	rule := MetricRule{Name: "server_cpu", Type: GaugeMetricType, ErrorMode: ErrorModeFail, Items: ".servers[]", Expression: ".cpu"}
...
func TestRootIsTheDocumentWithoutItems(t *testing.T) {
	set, err := itemsSet(t, "jq", "application/json", itemsDocument, MetricRule{
		Name: "servers", Type: GaugeMetricType, Expression: "$root.servers | length", Labels: []LabelRule{exprLabel("site", ".site")},
	})
	if err != nil {
		t.Fatal(err)
	}
