...
func (s *Server) pushOTLP(resources []otlpResourceSet) {
	...
	total:=0;for _,resource:=range resources{total+=len(resource.Set.Metrics)}
	if !cfg.Enabled||cfg.Endpoint==""||total==0{return}
	...
	now:=strconv.FormatInt(time.Now().UnixNano(),10);payload:=otlpPayload{}
	for _,resource:=range resources{if len(resource.Set.Metrics)==0{continue};payload.ResourceMetrics=append(payload.ResourceMetrics,otlpResourceMetrics{Resource:otlpResource{Attributes:resource.Identity.attributes()},ScopeMetrics:[]otlpScopeMetrics{{Scope:otlpScope{Name:"prometheus-universal-exporter"},Metrics:otlpMetrics(resource.Set,now)}}})}
...
func otlpAttributesForLabels(labels map[string]string)[]otlpAttribute{out:=make([]otlpAttribute,0,len(labels));for _,k:=range sortedKeys(labels){out=append(out,otlpAttribute{Key:k,Value:otlpValue{StringValue:labels[k]}})};return out}
