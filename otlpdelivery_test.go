...
// otlpstatus.go, readiness.go, transport.go).
...
func otlpServer(t *testing.T, endpoint string) *Server {
	t.Helper()
	cfg := &Config{Collectors: []Collector{testCollector("text", "text")}, OTLP: otlpConfig(endpoint + "/v1/metrics")}
...
func queueProbeMetric(server *Server, name string, value float64) {
	server.queueOTLP(MetricSet{Metrics: []Metric{{Name: name, Type: GaugeMetricType, Value: value}}})
}
...
func TestOTLPExportsAreGzippedUnlessCompressionIsNone(t *testing.T) {
	...
	if got := server.manager.Get().OTLP.Compression; got != OTLPCompressionGzip {
	...
	server.manager.Get().OTLP.Compression = OTLPCompressionNone
	server.exportOTLP(context.Background(), 5*time.Second)
...
func TestOTLPCompressionIsValidated(t *testing.T) {
	...
	if err := cfg.Validate(); err == nil || !strings.Contains(err.Error(), "otlp.compression") {
...
func TestTheLastOTLPExportAtShutdown(t *testing.T) {
	...
	server.manager.Get().OTLP.Interval = Duration(10 * time.Millisecond)
	...
	waitFor(t, "the loop to start an export", func() bool { return requests.Load() == 1 })
	cancel()
	select {
	case <-done:
...
func TestNotReadyWhileTheConfigurationIsRejected(t *testing.T) {
	...
	r.write(r.path, collectorsDocument("first"))
	if err := r.manager.Reload("http"); err != nil {
		t.Fatal(err)
	}
