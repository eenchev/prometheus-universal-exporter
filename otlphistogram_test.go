...
func TestHistogramsAreExportedAsOTLPHistograms(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"with a +Inf bucket": {
			buckets: []Bucket{{0.1, 2}, {1, 5}, {math.Inf(1), 7}}, count: 7,
			bounds: []otlpDouble{0.1, 1}, counts: []string{"2", "3", "2"},
		},
		"without one": {
			buckets: []Bucket{{0.1, 2}, {1, 5}}, count: 9,
			bounds: []otlpDouble{0.1, 1}, counts: []string{"2", "3", "4"},
		},
		"out of order": {
			buckets: []Bucket{{1, 5}, {0.1, 2}}, count: 5,
			bounds: []otlpDouble{0.1, 1}, counts: []string{"2", "3", "0"},
		},
		"a count that falls": {
			buckets: []Bucket{{0.1, 4}, {1, 3}}, count: 4,
...
func TestSeriesOfAFamilyShareOneOTLPMetric(t *testing.T) {
	out := encodeOTLP(t,
		...
		Metric{Name: "bare", Type: HistogramMetricType, Value: 4},
	)
	if len(out) != 3 {
...
func TestNonFiniteValuesEncode(t *testing.T) {
	...
	raw, _ := json.Marshal(otlpMetrics(MetricSet{Metrics: []Metric{{Name: "n", Type: GaugeMetricType, Value: math.NaN()}}}, "1"))
...
func TestHistogramsAndSummariesReachTheOTLPEndpoint(t *testing.T) {
	...
	c := testCollector("passthrough", "prometheus")
	c.Transform = TransformConfig{Type: "prometheus"}
	c.Metrics = nil
	cfg := &Config{Collectors: []Collector{c}, OTLP: otlpConfig(endpoint.URL + "/v1/metrics"), Web: WebConfig{SelfMetrics: SelfMetricsConfig{Verbose: true, ResourceMetrics: true}}}
	file := &TargetFile{Targets: []ScheduledTarget{{Name: "scheduled", Collector: "passthrough", Target: target.URL}}}
