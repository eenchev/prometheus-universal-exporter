...
func flightServer(t *testing.T, collectors ...Collector) *Server {
	t.Helper()
...
func TestConcurrentIdenticalProbesShareOneRequest(t *testing.T) {
	...
	server := flightServer(t, testCollector("shared", "text"))
...
func TestProbesThatDoNotOverlapAreNotShared(t *testing.T) {
	...
	target := newGatedTarget(t, http.StatusOK, "value=42\n")
	target.open()
	server := flightServer(t, testCollector("sequential", "text"))
	for i := 0; i < 2; i++ {
...
func TestASharedFailureReachesEveryProbe(t *testing.T) {
	...
	server := flightServer(t, testCollector("failing", "text"))
...
func TestASharedMetricFailureKeepsItsJSONBody(t *testing.T) {
	...
	c := testCollector("strict", "text")
	c.Metrics[0].ErrorMode = ErrorModeFail
	server := flightServer(t, c)
	var outcomes []<-chan probeOutcome
	for i := 0; i < 2; i++ {
...
func TestTheFirstProbeLeavingDoesNotFailTheOthers(t *testing.T) {
	captureLogs(t)
	target := newGatedTarget(t, http.StatusOK, "value=42\n")
	server := flightServer(t, testCollector("leader", "text"))
...
func TestTheRequestIsCancelledWhenEveryProbeHasGone(t *testing.T) {
	captureLogs(t)
	target := newGatedTarget(t, http.StatusOK, "value=42\n")
	server := flightServer(t, testCollector("abandoned", "text"))
...
func TestCoalescingCanBeTurnedOff(t *testing.T) {
	captureLogs(t)
	target := newGatedTarget(t, http.StatusOK, "value=42\n")
	c := testCollector("independent", "text")
	off := false
	c.Coalesce = &off
	server := flightServer(t, c)
	var outcomes []<-chan probeOutcome
