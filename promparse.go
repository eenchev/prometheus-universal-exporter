...
func parsePrometheusText(body []byte) ([]Metric, error) {
...
type promFamily struct {
	...
	help    string
	helpSet bool
	typ     MetricType // empty until a TYPE line or the first sample
	series  []*promSeries
...
func (p *promParser) comment(s string) error {
	...
	case "counter":
		family.typ = CounterMetricType
	case "gauge":
		family.typ = GaugeMetricType
	case "summary":
		family.typ = SummaryMetricType
	case "histogram":
		family.typ = HistogramMetricType
	case "untyped":
		family.typ = UntypedMetricType
	default:
...
func (p *promParser) family(name string) (*promFamily, int, error) {
	...
	if f := p.byName[base]; f != nil && f.typ == SummaryMetricType {
		return f, role, nil
	}
	if role == promRoleNone && len(name) > len("_bucket") && strings.HasSuffix(name, "_bucket") {
		base = strings.TrimSuffix(name, "_bucket")
	}
	if f := p.byName[base]; f != nil && f.typ == HistogramMetricType {
		return f, role, nil
	}
...
func (f *promFamily) add(labels []promLabel, role int, value float64, timestamp *int64) error {
	...
	case series.summary != nil:
		series.summary.Quantiles = append(series.summary.Quantiles, Quantile{Quantile: bound, Value: value})
	default:
		series.histogram.Buckets = append(series.histogram.Buckets, Bucket{UpperBound: bound, CumulativeCount: uint64(value)})
	}
	return nil
}

func (p *promParser) metrics() []Metric {
	var out []Metric
	for _, f := range p.families {
		for _, s := range f.series {
			m := Metric{Name: f.name, Help: f.help, Type: f.typ, Labels: s.labels, Value: s.value, Timestamp: s.timestamp, Histogram: s.histogram, Summary: s.summary}
			out = append(out, m)
		}
	}
	return out
