...
func parseOK(t *testing.T, body string) []Metric {
	t.Helper()
...
`)
	...
	first := metrics[0]
	if first.Name != "requests_total" || first.Type != CounterMetricType || first.Value != 42 ||
	...
	if metrics[2].Name != "temperature" || metrics[2].Type != GaugeMetricType || metrics[2].Value != -1.5 {
		t.Fatalf("third=%#v", metrics[2])
	}
	// A family without a TYPE line is untyped.
	if metrics[3].Name != "plain" || metrics[3].Type != UntypedMetricType || metrics[3].Value != 7 {
...
`)
	...
	a := metrics[0]
	if a.Name != "latency_seconds" || a.Type != HistogramMetricType || a.Labels["path"] != "/a" || len(a.Labels) != 1 {
		t.Fatalf("a=%#v", a)
	}
	want := &Histogram{Sum: 2.5, Count: 5, Buckets: []Bucket{{0.1, 1}, {1, 4}, {math.Inf(1), 5}}}
...
`)
	...
	s := metrics[0].Summary
	if metrics[0].Type != SummaryMetricType || s == nil || s.Sum != 12 || s.Count != 40 || len(s.Quantiles) != 2 ||
		s.Quantiles[0] != (Quantile{0.5, 0.2}) || s.Quantiles[1].Quantile != 0.99 || !math.IsNaN(s.Quantiles[1].Value) {
...
`)
	...
	if !reflect.DeepEqual(names, []string{"jobs_total", "jobs_count"}) || metrics[0].Type != UntypedMetricType {
		t.Fatalf("metrics=%#v", metrics)
	}
}
