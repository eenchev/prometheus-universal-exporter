...
func (s *Server) reconcile() {
	...
	for i := range cfg.Collectors {
		current[cfg.Collectors[i].Name] = collectorFingerprint(&cfg.Collectors[i])
	}
	...
	s.logger.Debug("per-collector state follows the reloaded configuration", "removed", sortedNames(removed), "changed", len(stale)-len(removed), "cache_entries_dropped", dropped)
}
