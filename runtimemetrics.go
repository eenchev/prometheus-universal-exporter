...
// switched off.
func (s *Server) runtimeMetrics() []Metric {
...
func gauge(name, help string, value float64, labels map[string]string) Metric {
	return Metric{Name: name, Help: help, Type: GaugeMetricType, Value: value, Labels: labels}
}

func counter(name, help string, value float64) Metric {
	return Metric{Name: name, Help: help, Type: CounterMetricType, Value: value}
}

// goMetrics are the runtime's own numbers. Every one of these is available on
// every platform the exporter builds for.
func goMetrics() []Metric {
