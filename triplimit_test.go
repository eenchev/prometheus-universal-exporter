...
func TestMaxConcurrentProbesDefaultsAndValidation(t *testing.T) {
	...
	if err := (&Config{Collectors: []Collector{c}}).Validate(); err == nil || !strings.Contains(err.Error(), "max_concurrent_probes must not be negative") {
		t.Fatalf("err=%v", err)
	}
}
...
func TestProbesOverTheLimitAreRejected(t *testing.T) {
	...
	waitFor(t, "two probes at the target", func() bool { return target.requests() == 2 })
	...
	waitFor(t, "the slots to be freed", func() bool { return server.trips.count("limited") == 0 })
...
func TestSharedAndCachedProbesTakeNoSlot(t *testing.T) {
	...
	waitFor(t, "the first probe at the target", func() bool { return target.requests() == 1 })
	go func() { results <- probe("/same") }()
	waitFor(t, "the second probe to join", func() bool {
	...
	waitFor(t, "the slot to be taken", func() bool { return server.trips.count("single") == 1 })
...
func TestScheduledTargetsWaitForASlot(t *testing.T) {
	...
	c := testCollector("shared_limit", "text")
	c.MaxConcurrentProbes = 1
	cfg := &Config{Collectors: []Collector{c}, OTLP: otlpConfig("http://collector.invalid/v1/metrics")}
	file := &TargetFile{Targets: []ScheduledTarget{{Name: "scheduled", Collector: "shared_limit", Target: scheduled.URL}}}
	...
	waitFor(t, "the probe to take the slot", func() bool { return server.trips.count("shared_limit") == 1 })
