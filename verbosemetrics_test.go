...
func TestTargetScrapeHistogramIsPublishedWhenVerbose(t *testing.T) {
	...
	server := verboseServer(t, true, testCollector("timed", "text"), testCollector("idle", "text"))
	for i := 0; i < 2; i++ {
...
func TestOnlyTripsToTheTargetAreObserved(t *testing.T) {
	...
	shared := verboseServer(t, true, testCollector("shared_timed", "text"))
	...
	scheduled := testCollector("scheduled_timed", "text")
	cfg := &Config{Collectors: []Collector{scheduled}, OTLP: otlpConfig("http://collector.invalid/v1/metrics"), Web: WebConfig{SelfMetrics: SelfMetricsConfig{Verbose: true}}}
	server = newScheduledServer(t, cfg, &TargetFile{Targets: []ScheduledTarget{{Name: "eu", Collector: "scheduled_timed", Target: target.URL}}})
...
func TestTheHistogramIsNotRecordedWhenNotVerbose(t *testing.T) {
	captureLogs(t)
	target := textTarget(t, "value=42\n")
	server := verboseServer(t, false, testCollector("untimed", "text"))
...
func pythonCollector(name, script string) Collector {
	return Collector{
		Name: name, Request: RequestConfig{Type: RequestTypeHTTP}, Response: ResponseConfig{Format: "text"},
		Transform: TransformConfig{Type: "python", Script: script},
		Limits:    Limits{ScriptTimeout: Duration(2 * time.Second)},
	}
}
...
func TestPythonWorkerMetrics(t *testing.T) {
	...
	slow.Limits.ScriptTimeout = Duration(100 * time.Millisecond)
	crashing := pythonCollector("py_metrics_crash", "import os\nos._exit(1)\n")
	server := verboseServer(t, true, good, failing, slow, crashing, testCollector("py_metrics_none", "text"))
...
func TestAPassedThroughHistogramHasOneInfBucket(t *testing.T) {
	...
	server := verboseServer(t, false, Collector{Name: "passthrough", Request: RequestConfig{Type: RequestTypeHTTP}, Transform: TransformConfig{Type: "prometheus"}})
