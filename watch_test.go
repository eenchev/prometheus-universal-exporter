...
func watchedManager(t *testing.T) (*ConfigManager, string) {
	t.Helper()
...
func activeMetric(m *ConfigManager) string { return m.Get().Collectors[0].Metrics[0].Name }

func waitForMetric(t *testing.T, m *ConfigManager, want string, within time.Duration) bool {
	t.Helper()
